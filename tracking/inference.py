...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for var in variables:
        if var in [PAC, GHOST0, GHOST1]:
            variableDomainsDict[var] = positions
        else:
            variableDomainsDict[var] = [obs for obs in range(maxDistance + MAX_NOISE + 1)]
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    # this is for autograding -- don't modify
        ...
        if eliminationOrder is None: 
            # set an arbitrary elimination order if None given
            eliminationVariables = bayesNet.variablesSet() - set(queryVariables) - set(evidenceDict.keys())
            eliminationOrder = sorted(list(eliminationVariables))

        # obtain all the factors (CPTs) given the evidence
        ...
        for var in eliminationOrder:
            ...
            if factors_with_var:
                factors_to_not_join, joined_factor = joinFactorsByVariable(factors_with_var, var)

                if var not in queryVariables and var not in evidenceDict:
                    # Don't eliminate if it is the only unconditioned variable in joined_factor
                    if len(joined_factor.unconditionedVariables()) > 1:
                        eliminated_factor = eliminate(joined_factor, var)
                        factors = factors_to_not_join + [eliminated_factor]
                    else:
                        factors = factors_to_not_join + [joined_factor]
                else:
                    factors = factors_to_not_join + [joined_factor]
            else:
                factors_to_not_join = [f for f in factors if var not in f.variablesSet()]
                factors = factors_to_not_join

        # join remaining factors to obtain the final factor
        final_factor = joinFactors(factors)

        # normalize the final factor so the probabilities sum up to 1
        total_probability = sum(final_factor.getProbability(a) for a in final_factor.getAllPossibleAssignmentDicts())
        for a in final_factor.getAllPossibleAssignmentDicts():
            final_factor.setProbability(a, final_factor.getProbability(a) / total_probability)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        random_value = random.random() * sum(self.values())
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            ...
            updatedBeliefs[position] = observationProb * self.beliefs[position]
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in oldBeliefs:
            ...
            for newPos, prob in newPosDist.items():
                self.beliefs[newPos] += oldBeliefs[oldPos] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for _ in range(self.numParticles // numLegalPositions):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        # Compute weights based on the observation probabilities
        for particle in self.particles:
        ...
        if weights.total() == 0:
            ...
            self.particles = [weights.sample() for _ in range(len(self.particles))]
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            ...
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            # Sample a new position from newPosDist

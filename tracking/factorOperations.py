...
def joinFactors(factors: List[Factor]):
    ...
    "*** YOUR CODE HERE ***"
    factors = list(factors)  # Convert dict_values object to list
    unconditioned = set()
    conditioned = set()
    variableDomainsDict = factors[0].variableDomainsDict()  # Assume all factors have the same variableDomainsDict
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        # Remove eliminationVariable from the unconditioned variables set
        unconditioned.remove(eliminationVariable)

        # Create a new factor without the eliminationVariable
        new_factor = Factor(unconditioned, conditioned, variableDomainsDict)

        # For each assignment in the new factor, its probability is the sum of the probabilities
        # in the original factor of the assignments that match on the other variables
        for assignment in new_factor.getAllPossibleAssignmentDicts():
            probability = 0
